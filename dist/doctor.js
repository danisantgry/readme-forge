...
async function findReadmeWorkflow(root) {
    ...
    for (const entry of entries) {
        ...
        if (source.includes("readme-forge"))
            return workflowPath;
    }
    return undefined;
}
function relativeCommandPath(root, targetPath) {
...
function installRecommendation(facts) {
...
function scriptCommand(facts, script) {
    if (facts.packageManager === "pnpm")
        return `pnpm ${script}`;
    if (facts.packageManager === "yarn")
        return `yarn ${script}`;
...
function detectedShape(facts) {
    if (facts.frameworks.some((name) => ["Vite", "Next.js", "React"].includes(name)))
        return "web";
    if (facts.frameworks.some((name) => ["Python package", "Rust crate", "Go module"].includes(name)))
        return "library";
    if (facts.scripts.dev || facts.scripts.start || facts.files.includes("src"))
        return "cli";
    return "library";
}
...
function ecosystemRecommendations(report) {
