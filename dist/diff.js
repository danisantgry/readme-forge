...
function positionalDiff(oldLines, newLines) {
    const result = [];
    ...
    for (let index = 0; index < max; index += 1) {
        ...
        if (oldValue !== undefined)
            result.push({ type: "removed", value: oldValue, oldLine: index + 1 });
        if (newValue !== undefined)
            result.push({ type: "added", value: newValue, newLine: index + 1 });
    }
    return result;
}
export function diffLines(existing, generated) {
...
export function formatUnifiedDiff(existing, generated) {
    const diff = diffLines(existing, generated);
    const summary = summarizeDiff(diff);
    if (!summary.additions && !summary.removals)
        return "README is already in sync with generated output.";
    return [
        "--- README.md",
        "+++ generated README.md",
