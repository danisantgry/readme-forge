...
function usageSection(facts: ProjectFacts, selectedPreset: Exclude<TemplatePreset, "auto">): string {
  if (!facts.binCommands.length) return "";
...
function configurationSection(facts: ProjectFacts): string {
  const lines: string[] = [];
...
function entrypointSection(facts: ProjectFacts): string {
  if (!facts.entrypoints.length) return "";
  return `\n## Package Entrypoints\n\n${facts.entrypoints.map((entrypoint) => `- \`${entrypoint}\``).join("\n")}\n`;
}

function automationSection(facts: ProjectFacts): string {
  const lines: string[] = [];
